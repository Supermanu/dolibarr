...
class FormActions
{
	...
	public function showactions($object, $typeelement, $socid = 0, $forceshowtitle = 0, $morecss = 'listactions', $max = 0, $moreparambacktopage = '', $morehtmlcenter = '')
	{
		...
		if ($num || $forceshowtitle)
		{
			...
			{
				$newcardbutton .= dolGetButtonTitle($langs->trans("AddEvent"), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/comm/action/card.php?action=create&amp;datep='.urlencode(dol_print_date(dol_now(), 'dayhourlog', 'auto')).'&amp;origin='.urlencode($typeelement).'&amp;originid='.$object->id.($object->socid > 0 ? '&amp;socid='.$object->socid : ($socid > 0 ? '&amp;socid='.$socid : '')).($projectid > 0 ? '&amp;projectid='.$projectid : '').'&amp;backtopage='.urlencode($urlbacktopage));
			}
			...
			if (is_array($listofactions) && count($listofactions))
			{
				...
				foreach ($listofactions as $actioncomm)
				{
					...
					// Date
					print '<td class="center">'.dol_print_date($actioncomm->datep, 'dayhour');
					if ($actioncomm->datef)
					{
						...
						{
							if ($tmpa['hours'] != $tmpb['hours'] || $tmpa['minutes'] != $tmpb['minutes'] && $tmpa['seconds'] != $tmpb['seconds']) print '-'.dol_print_date($actioncomm->datef, 'hour', 'auto');
						} else print '-'.dol_print_date($actioncomm->datef, 'dayhour');
					}
					print '</td>';
